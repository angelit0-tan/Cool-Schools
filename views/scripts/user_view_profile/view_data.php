...
$(document).ready(function(){
	...
	$('#i_post').click(function(){
		...
		$.get('index.php?__profile/vprepend_post&m_i_stat='+fix_txt+'&id='+$('#f_id').val(),function(data, response){
			if (response =='success'){
			var json_obj = $.parseJSON(data);
				...
				$("<div id='_cid'"+json_obj.id+"style='height: auto; margin-bottom: 10px; margin-top: 10px; border-bottom-width: 1px; border-bottom-style: solid; border-bottom-color: rgb(238, 238, 238);'>"+
					...
					"<tr>"+
						"<td style='align: center; top: middle; position: absolute;'>"+
							"<img style='height: 50px; width: 50px;' src= 'index.php?__profile/s_list&id=<?php echo $_SESSION['user_id'];?>' >" +
						"</td>"+
	...
	$('.d_cmnt').click(function(){
		...
		$.post('index.php?__profile/u_del_cmnt', { comment_id : comment_id[0] }, function(data, response){
			if ( response ){
				$('#c_area_'+comment_id[0]+'_'+$('#i_h_id').val()).remove();
			}
		});
	});
	//hover wall
...
?>
	<div class='wall_area' id="cid_<?php echo $row['wall_id'].'_'.$row['user_id'];?>" style='height: auto; margin-bottom: 10px; margin-top: 10px; border-bottom-width: 1px; border-bottom-style: solid; border-bottom-color: rgb(238, 238, 238);'>
				...
				<div style='margin-left: 55px;'>
					...
					<a href="index.php?__profile/profile&id=<?php echo $row['from_id'];?>"> <b> <?php echo ucfirst($row['firstname']) . ' ' . ucfirst($row['lastname']); ?> </b> </a>
						...
						<div style='width: 95%;background-color: #F0F0FF; height: auto;'>
							...
							?>
								<div class='comment-area' id="c_area_<?php echo $row2['w_c_id'].'_'.$row2['user_id'];?>" style='background-color: #F0F0FF; height: auto; border-bottom-width: 1px; border-bottom-style: solid; border-bottom-color: #C4DBD4;'>
...
?>
<input type='hidden' id='i_h_id' value="<?php echo $_SESSION['user_id']; ?>">
</div>
